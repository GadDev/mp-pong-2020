 ...
 * stays foreground.
 *
 * Layered on top of that is a generative structure rather than a looping
 * track: Layer A (machine drone, above) is always present; Layer B (pulse)
 * and Layer C (harmony) fade in as the cumulative rally count rises, so a
 * long match is audibly "becoming music" instead of a track starting on
 * Play; Layer D (disturbance) is a rare unscheduled swell. A lost rally
 * doesn't snap the pulse tempo back to silence — it decays over a few
 * seconds, so the music exhales instead of resetting.
 */
export class RevealAudio {
  ...
  setActive(active: boolean): void {
    ...
    if (!active) {
      ...
      this.pulseAmpGain.gain.setTargetAtTime(0, t, 0.1);
      this.harmonyGain.gain.setTargetAtTime(0, t, 0.1);
      this.shimmerGain.gain.setTargetAtTime(0, t, 0.1);
    }
  }
